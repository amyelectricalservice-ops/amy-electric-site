...
main() {
  ...
  if [ ! -d "$SOURCE_DIR" ] || [ -z "$(ls -A "$SOURCE_DIR" 2>/dev/null)" ]; then
    warn "No images found in ${SOURCE_DIR}"
    exit 0
  fi
...
EOF
  ...
  while IFS= read -r -d '' src; do
    ...
    local folder_name=$(basename "$(dirname "$src")")
    local category=$(detect_category "$src_basename $folder_name")
    local title=$(generate_title "$src_basename")
    ...
    log "  [${category}] ${src_basename} → ${new_name}"
  done < <(find "$SOURCE_DIR" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.gif" -o -iname "*.webp" \) -print0 | sort -z)
