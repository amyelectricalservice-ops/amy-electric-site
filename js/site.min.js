...
) .then(r => r.ok ? showSuccess() : showFallback(data)) .catch(() => showFallback(data));
 }
 else {
 showFallback(data);
 }
...
data.message}
`;
 window.location.href = `mailto:info@amyelectric.com?subject=${
encodeURIComponent(requestType + ' - ' + data.name)}
&body=${
